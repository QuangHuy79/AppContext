...
// ==========================================
// Đây là phiên bản APIContext.jsx sau khi:
// ❌ Bỏ toàn bộ console.error
// ❌ Bỏ success toast mặc định
// ✅ Giữ error toast có kiểm soát
// ✅ Giữ nguyên toàn bộ kiến trúc C-2 / C-3 / Phase 3.7
// ❌ Không thêm logic mới, chỉ dọn sạch & khóa invariant
// src/context/APIContext/APIContext.jsx
import React, {
  createContext,
  useContext,
 ...
 * Production-ready APIProvider
 *
 * - Stable API object (memoized)
 * - Abortable requests
 * - No internal state exposed
 * - No UI side-effects except controlled error toast
 */
export const APIProvider = ({ children }) => {
  ...
  /* --------------------------------------------------
     Core request (abortable, UI-safe)
  -------------------------------------------------- */
  const request = useCallback(
    async (endpoint, options = {}, showToast = true) => {
      const controller = options.signal ? null : new AbortController();
  ...
  const get = useCallback(
    (endpoint, showToast = true, options = {}) =>
      request(endpoint, { ...options, method: "GET" }, showToast),
    [request]
  );
  ...
  const del = useCallback(
    (endpoint, showToast = true, options = {}) =>
      request(endpoint, { ...options, method: "DELETE" }, showToast),
    [request]
  );
  ...
  const api = useMemo(
    ...
    [request, requestWithAuth, get, post, put, del, createAbortController]
  );
